...
def _format_cli_exception(exc: Exception) -> str:
    ...
    chain_parts = [f"{type(item).__name__}: {item}" for item in chain if str(item)]
    if not chain_parts:
        chain_parts = [type(exc).__name__]

    details = [f"Cause chain: {' -> '.join(chain_parts)}"]

    deepest = chain[-1] if chain else exc
    extracted = (
        traceback.extract_tb(deepest.__traceback__) if deepest.__traceback__ else []
    )
    if extracted:
        frame = extracted[-1]
        details.append(
            f"Traceback location: {Path(frame.filename).name}:{frame.lineno} in {frame.name}"
        )
        if frame.line:
            details.append(f"Failing line: {frame.line.strip()}")

    return "\n".join(details)


def _print_cli_exception(prefix: str, exc: Exception) -> None:
    console.print(f"[bold red]{prefix}[/bold red] {exc}")
    console.print(f"[red]{_format_cli_exception(exc)}[/red]")
    error_logger(traceback.format_exc())


def _format_job_failure(job) -> str:
    summary = job.errors[0] if job.errors else "Unknown error"
    details = [summary]

    if job.error_type:
        details.append(f"Error type: {job.error_type}")
    if job.current_phase:
        details.append(f"Job phase: {job.current_phase}")
    if job.current_file:
        details.append(f"Current item: {job.current_file}")
    if job.error_details:
        details.append("Original traceback:")
        details.append(job.error_details.rstrip())
...
def _initialize_services():
    ...
    try:
        ...
        if isinstance(e, FalkorDBUnavailableError):
            console.print(
                f"[yellow]⚠ FalkorDB Lite is not functional in this environment: {e}[/yellow]"
            )
            console.print(
            ...
            try:
                ...
                console.print(
                    f"[bold red]Critical Error:[/bold red] Both FalkorDB and KùzuDB failed: {kuzu_e}"
                )
                return None, None, None
        else:
            console.print(f"[bold red]Database Connection Error:[/bold red] {e}")
            console.print(
                "Please ensure your database is configured correctly or run 'cgc doctor'."
            )
            return None, None, None
...
async def _run_index_with_progress(
    graph_builder: GraphBuilder, path_obj: Path, is_dependency: bool = False
):
    """Internal helper to run indexing with a Live progress bar."""
    job_id = graph_builder.job_manager.create_job(
        str(path_obj), is_dependency=is_dependency
    )
    ...
    with Progress(
        ...
        while indexing_thread.is_alive():
            ...
            if job:
                phase = job.current_phase or "indexing"
                ...
                if snapshot != last_snapshot:
                    update_kwargs = {
                        "description": f"{phase.replace('_', ' ').title()}...",
                        "filename": current_file,
                        "detail": detail,
                    }
                    if job.total_files > 0:
                        update_kwargs["total"] = job.total_files
                        update_kwargs["completed"] = job.processed_files
        ...
        if job and job.status == JobStatus.FAILED:
            raise RuntimeError(_format_job_failure(job))
...
def index_helper(path: str):
    ...
    if repo_exists:
        ...
        try:
            with db_manager.get_driver().session() as session:
                ...
                if file_count > 0:
                    console.print(
                        f"[yellow]Repository '{path}' is already indexed with {file_count} files. Skipping.[/yellow]"
                    )
                    console.print(
                        "[dim]💡 Tip: Use 'cgc index --force' to re-index[/dim]"
                    )
                    db_manager.close_driver()
                    return
                else:
                    console.print(
                        f"[yellow]Repository '{path}' exists but has no files (likely interrupted). Re-indexing...[/yellow]"
                    )
        except Exception as e:
            console.print(
                f"[yellow]Warning: Could not check file count: {e}. Proceeding with indexing...[/yellow]"
            )
    ...
    try:
        asyncio.run(
            _run_index_with_progress(graph_builder, path_obj, is_dependency=False)
        )
        time_end = time.time()
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not check ENABLE_AUTO_WATCH: {e}[/yellow]"
            )

    except Exception as e:
        _print_cli_exception("An error occurred during indexing:", e)
    finally:
        db_manager.close_driver()
...
def add_package_helper(package_name: str, language: str):
    ...
    if not package_path_str:
        console.print(
            f"[red]Error: Could not find package '{package_name}' for language '{language}'.[/red]"
        )
        db_manager.close_driver()
        return
    ...
    try:
        asyncio.run(
            _run_index_with_progress(graph_builder, package_path, is_dependency=True)
        )
        console.print(
        ...
        console.print(
            f"[bold red]An error occurred during package indexing:[/bold red] {e}"
        )
    finally:
        db_manager.close_driver()
...
def delete_helper(repo_path: str):
    ...
    try:
        if graph_builder.delete_repository_from_graph(repo_path):
            ...
            console.print(
                "[dim]Tip: Use 'cgc list' to see available repositories.[/dim]"
            )
    except Exception as e:
        console.print(f"[bold red]An error occurred:[/bold red] {e}")
    finally:
...
def cypher_helper(query: str):
    ...
    try:
        ...
        console.print(
            f"[bold red]An error occurred while executing query:[/bold red] {e}"
        )
    finally:
        db_manager.close_driver()
...
def visualize_helper(repo_path: Optional[str] = None, port: int = 8000):
    ...
    if not static_dir.exists():
        console.print(
            "[yellow]Warning: Visualizer UI assets not found in package. Using fallback static dir.[/yellow]"
        )
    ...
    try:
        ...
        console.print(
            f"[bold red]An error occurred while running the server:[/bold red] {e}"
        )
    finally:
        db_manager.close_driver()
    ...
    try:
        ...
        with db_manager.get_driver().session() as session:
            ...
            for record in result:
                ...
                data_edges.append(
                    {"from": nid, "to": mid, "label": e_type, "arrows": "to"}
                )
...
def reindex_helper(path: str):
    ...
    try:
        ...
        _print_cli_exception("An error occurred during re-indexing:", e)
    finally:
        db_manager.close_driver()
    ...
    try:
        if path:
            ...
            with db_manager.get_driver().session() as session:
                ...
                module_count = session.run(module_query, path=str(path_obj)).single()[
                    "c"
                ]
            ...
            with db_manager.get_driver().session() as session:
                ...
                if repo_count > 0:
                    ...
                    func_count = session.run(
                        "MATCH (f:Function) RETURN count(f) as c"
                    ).single()["c"]
                    class_count = session.run(
                        "MATCH (c:Class) RETURN count(c) as c"
                    ).single()["c"]
...
def watch_helper(path: str):
    ...
    try:
        ...
        console.print(
            "[bold green]👀 Monitoring for file changes...[/bold green] (Press Ctrl+C to stop)"
        )
        console.print(
...
def unwatch_helper(path: str):
    ...
    console.print(
        f"[yellow]⚠️  Note: 'cgc unwatch' only works when the watcher is running via MCP server.[/yellow]"
    )
    console.print(
        f"[dim]For CLI watch mode, simply press Ctrl+C in the watch terminal.[/dim]"
    )
...
def list_watching_helper():
    ...
    console.print(
        f"[yellow]⚠️  Note: 'cgc watching' only works when the watcher is running via MCP server.[/yellow]"
    )
    console.print(
        f"[dim]For CLI watch mode, check the terminal where you ran 'cgc watch'.[/dim]"
    )
