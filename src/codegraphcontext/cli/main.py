...
def _load_credentials():
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning: Could not load .env from current directory: {e}[/yellow]"
        )
    ...
    for key, value in merged_config.items():
        if value is not None:  # Only set non-None values
            # Never let .env clobber a DB-selection or DB-credential key that the user already set in the shell.
            # This is especially important for one-off connection overrides such as
            # `NEO4J_URI=bolt://localhost:7687 cgc index ...`.
    ...
    if config_source_names:
        if len(config_source_names) == 1:
            ...
            console.print(
                f"[dim]Loaded configuration from: {', '.join(config_source_names)} (highest priority: {config_source_names[-1]})[/dim]"
            )
    else:
...
def config_db(
    backend: str = typer.Argument(..., help="Database backend: 'neo4j' or 'falkordb'")
):
    """
...
def bundle_import(
    bundle_file: str = typer.Argument(
        ..., help="Path to the .cgc bundle file to import"
    ),
    ...
    try:
        ...
        bundle = CGCBundle(db_manager)
        success, message = bundle.import_from_bundle(bundle_path, clear_existing=clear)
...
def bundle_load(
    bundle_name: str = typer.Argument(
        ..., help="Bundle name or path to load (e.g., 'numpy' or 'numpy.cgc')"
    ),
    clear: bool = typer.Option(
    ...
    if bundle_path.is_absolute() or (
        bundle_path.suffix == ".cgc" and bundle_path.exists()
    ):
...
def registry_list(
    ...
    unique: bool = typer.Option(
        False, "--unique", "-u", help="Show only one version per package (most recent)"
    ),
):
    """
...
def registry_download(
    ...
    load: bool = typer.Option(
        False, "--load", "-l", help="Automatically load the bundle after downloading"
    ),
):
    """
...
def doctor():
    ...
    try:
        ...
        if config_manager.CONFIG_FILE.exists():
            console.print(
                f"   [green]✓[/green] Configuration loaded from {config_manager.CONFIG_FILE}"
            )
        else:
            console.print(f"   [yellow]ℹ[/yellow] No config file found, using defaults")
            console.print(
                f"   [dim]Config will be created at: {config_manager.CONFIG_FILE}[/dim]"
            )
        ...
        if default_db == "neo4j":
            ...
            if uri and username and password:
                ...
                is_connected, error_msg = DatabaseManager.test_connection(
                    uri, username, password, database=os.environ.get("NEO4J_DATABASE")
                )
                if is_connected:
                    console.print(f"   [green]✓[/green] Neo4j connection successful")
                else:
                    console.print(
                        f"   [red]✗[/red] Neo4j connection failed: {error_msg}"
                    )
                    all_checks_passed = False
            else:
                console.print(
                    f"   [yellow]⚠[/yellow] Neo4j credentials not set. Run 'cgc neo4j setup'"
                )
        else:
            # FalkorDB
            try:
                ...
                console.print(
                    f"   [yellow]⚠[/yellow] FalkorDB Lite not installed (Python 3.12+ only)"
                )
    ...
    try:
        ...
        if config_dir.exists():
            ...
            console.print(
                f"   [yellow]⚠[/yellow] Config directory doesn't exist, will be created on first use"
            )
    except Exception as e:
    ...
    if cgc_path:
        ...
        console.print(
            f"   [yellow]⚠[/yellow] cgc command not in PATH (using python -m cgc)"
        )
...
def start():
    ...
    """
    console.print(
        "[yellow]⚠️  'cgc start' is deprecated. Use 'cgc mcp start' instead.[/yellow]"
    )
    mcp_start()
        ...
        try:
            ...
            for repo in repos:
                ...
                try:
                    ...
                    console.print(
                        f"[red]✗[/red] Failed to delete {repo.get('name', '')}: {e}"
                    )
...
@app.command()
def unwatch(path: str = typer.Argument(..., help="Path to stop watching")):
    """
...
def find_by_name(
    ...
    type: Optional[str] = typer.Option(
        None, "--type", "-t", help="Filter by type (function, class, file, module)"
    ),
    ...
    try:
        ...
        if type is None or type.lower() == "all":
            ...
            for m in modules:
                m["type"] = "Module"
                m["path"] = m.get("name", "External")  # Modules might differ
            for i in imports:
                i["type"] = "Import"
            ...
            with db_manager.get_driver().session() as session:
                res = session.run(
                    "MATCH (n:File) WHERE n.name = $name RETURN n.name as name, n.path as path, n.is_dependency as is_dependency",
                    name=name,
                )
        ...
        for res in results:
            ...
            table.add_row(res.get("name", ""), res.get("type", "Unknown"), location_str)
...
def find_by_pattern(
    ...
    try:
        ...
        for res in results:
            path = res.get("path", "") or ""
            line_str = str(
                res.get("line_number", "") if res.get("line_number") is not None else ""
            )
            location_str = f"{path}:{line_str}" if line_str else path
...
def find_by_variable(
    ...
    try:
        ...
        for res in results:
            ...
            table.add_row(
                res.get("name", ""), location_str, res.get("context", "") or "module"
            )
    ...
    try:
        try:
            ...
            if (
                ...
                console.print(
                    f'     [dim]cgc --database neo4j find content "{query}"[/dim]\n'
                )
...
def find_by_decorator_search(
    ...
    try:
        ...
        for res in results:
            ...
            table.add_row(res.get("function_name", ""), location_str, decorators_str)
...
def find_by_argument_search(
    ...
    try:
        ...
        for res in results:
            ...
            table.add_row(res.get("function_name", ""), location_str)
...
def analyze_chain(
    ...
    to_file: Optional[str] = typer.Option(
    ...
    try:
        ...
        for idx, chain in enumerate(results, 1):
            ...
            for i, func in enumerate(functions):
                ...
                # Print function
                console.print(
                    f"{indent}[cyan]{func.get('name', 'Unknown')}[/cyan] [dim]({func.get('path', '')}:{func.get('line_number', '')})[/dim]"
                )
    ...
    try:
        ...
        if results.get("parent_classes"):
            ...
            for parent in results["parent_classes"]:
                console.print(
                    f"  ⬆ [cyan]{parent.get('parent_class', '')}[/cyan] [dim]({parent.get('parent_file_path', '')}:{parent.get('parent_line_number', '')})[/dim]"
                )
        else:
        ...
        if results.get("child_classes"):
            ...
            for child in results["child_classes"]:
                console.print(
                    f"  ⬇ [cyan]{child.get('child_class', '')}[/cyan] [dim]({child.get('child_file_path', '')}:{child.get('child_line_number', '')})[/dim]"
                )
        else:
    ...
    try:
        if path:
            ...
            if result:
                ...
                console.print(
                    f"  Cyclomatic Complexity: [yellow]{result.get('complexity', 'N/A')}[/yellow]"
                )
...
def analyze_dead_code(
    ...
    exclude_decorators: Optional[str] = typer.Option(
    ...
    try:
        ...
        for func in unused_funcs:
            ...
            table.add_row(func.get("function_name", ""), location_str)
...
def analyze_overrides(
    ...
    try:
        ...
        for res in results:
            ...
            table.add_row(
                res.get("class_name", ""), res.get("function_name", ""), location_str
            )
...
def cypher_legacy(
    ...
    _load_credentials()
    console.print(
        "[yellow]⚠️  'cgc cypher' is deprecated. Use 'cgc query' instead.[/yellow]"
    )
    cypher_helper(query)
...
def main(
    ...
    if ctx.invoked_subcommand is None:
        console.print(
            "[bold green]👋 Welcome to CodeGraphContext (cgc)![/bold green]\n"
        )
        console.print(
            "CodeGraphContext is both an [bold cyan]MCP server[/bold cyan] and a [bold cyan]CLI toolkit[/bold cyan] for code analysis.\n"
        )
        console.print("🤖 [bold]For MCP Server Mode (AI assistants):[/bold]")
        console.print(
            "   1. Run [cyan]cgc mcp setup[/cyan] (or [cyan]cgc m[/cyan]) to configure your IDE"
        )
        ...
        console.print(
            "     Run [cyan]cgc neo4j setup[/cyan] (or [cyan]cgc n[/cyan]) to configure Neo4j\n"
        )
        ...
        console.print(
            "👉 Running [green]codegraphcontext[/green] works the same as using [green]cgc[/green]"
        )
